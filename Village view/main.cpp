...
void cloud1() // eta nobobi set up krbe
{
    if (position1 > 1200)
    {
        position1=-185;
...
void myKey(unsigned char key, int x, int y)
{
    ...
    {
        case 'd':
            //sndPlaySound("day.wav",SND_ASYNC); //music
            glutDisplayFunc(dayDisplay);
            positionMoon = 0;
            break;

        case 'n':
            //sndPlaySound("night.wav",SND_ASYNC);//music
            glutDisplayFunc(nightDisplay);
            positionSun = 0;
            break;

        case 'r':
            //sndPlaySound("rain.wav",SND_ASYNC); //music
